 ...
 * Modified on: 07/01/2021 - version 2.8 - divide units, beta ver by Hasta
 */
...
(async function (TribalWars) {
    ...
    const i18n = {
        ...
        LABELS: {
            ...
            deff_count: 'Il. deffa',
            spear_count: 'Il. pik',
            sword_count: 'Il. miecz',
            heavy_count: 'Il. ck',
            spy_count: 'Il. zwiadu',
            village_count: 'Il. wiosek',
            minimal_deff_count: 'Minimalna il. deffa',
    ...
    const Guard = {
        ...
        create_main_panel: function () {
            const options = [
                ...
                { name: 'after_date', for: 'is_after_date_enabled', controls: [{ type: 'input', attributes: { id: 'is_after_date_enabled', type: 'checkbox' } }, { type: 'input', attributes: { id: 'after_date', size: 12 } }] },
        ...
        init_gui: async function () {
            ...
            for (const option_name of ['deff_count', 'spear_count', 'sword_count', 'heavy_count', 'spy_count', 'minimal_deff_count', 'village_count']) {
            ...
            const after_date = Helper.get_control('after_date');
            after_date.value = url_params.get('after_date') || `${default_date.getDate()}.${default_date.getMonth() + 1} ${default_date.getHours()}:00:00`;
            Helper.get_control('generate').addEventListener('click', async () => {
                try { await Guard.generate_commands(); } catch (ex) { Helper.handle_error(ex); }
            });
            ...
            const enable_after_date = Helper.get_control('is_after_date_enabled');
            enable_after_date.addEventListener('change', event => {
                Helper.get_control('after_date').disabled = !event.target.checked;
            });
            enable_after_date.disabled = false;
            Helper.get_control('generate').disabled = false;
            if (url_params.has('after_date')) {
                enable_after_date.checked = true;
                after_date.disabled = false;
            }
        },
        ...
        generate_commands: async function () {
            let get_user_input = function () {
                const user_input = {};
                const numeric_fields = ['deff_count', 'spear_count', 'sword_count', 'heavy_count', 'spy_count', 'village_count', 'minimal_deff_count'];
                ...
                if (Helper.get_control('is_after_date_enabled').checked) {
                    let after_date = Helper.parse_date(Helper.get_control('after_date').value, i18n.LABELS.after_date);
                    if (after_date.getTime() <= Date.now()) {
                        Helper.get_control('after_date').focus();
                        throw i18n.ERROR.PAST_DATE;
                    }
                    user_input.after_date_travel_time = (after_date.getTime() - Date.now()) / 60 / 1000;
                }
            ...
            let sort_by_spear_asc = function (lhs, rhs) {
                return lhs.units.spear !== rhs.units.spear ? lhs.units.spear > rhs.units.spear ? 1 : -1 : 0;
            };
            let sort_by_spear_desc = function (lhs, rhs) {
                return lhs.units.spear !== rhs.units.spear ? lhs.units.spear > rhs.units.spear ? -1 : 1 : 0;
            };

            let sort_by_sword_asc = function (lhs, rhs) {
                return lhs.units.sword !== rhs.units.sword ? lhs.units.sword > rhs.units.sword ? 1 : -1 : 0;
            };
            let sort_by_sword_desc = function (lhs, rhs) {
                return lhs.units.sword !== rhs.units.sword ? lhs.units.sword > rhs.units.sword ? -1 : 1 : 0;
            };

            let sort_by_heavy_asc = function (lhs, rhs) {
                return lhs.units.heavy !== rhs.units.heavy ? lhs.units.heavy > rhs.units.heavy ? 1 : -1 : 0;
            };
            let sort_by_heavy_desc = function (lhs, rhs) {
                return lhs.units.heavy !== rhs.units.heavy ? lhs.units.heavy > rhs.units.heavy ? -1 : 1 : 0;
            };
                ...
                if(user_input.deff_count !== 0) {
                    ...
                    troops_info.villages.sort(sort_by_spear_desc);
                        for (let i = troops_info.villages.length; i > 0; i--) {
                            ...
                            const ratio = threshold < village.units.spear ? threshold / village.units.spear : 1.0;
                            const selected_count = Math.min(Math.round(ratio * village.units.spear), village.units.spear);
                            troops_info.selected.spear += selected_count;
                            village.units.spear = selected_count;
                        }

                        troops_info.villages.sort(sort_by_sword_desc);
                        for (let i = troops_info.villages.length; i > 0; i--) {
                            const village = troops_info.villages[i - 1];
                            const threshold = (user_input.sword_count - troops_info.selected.sword) / i;
                            const ratio = threshold < village.units.sword ? threshold / village.units.sword : 1.0;
                            const selected_count = Math.min(Math.round(ratio * village.units.sword), village.units.sword);
                            troops_info.selected.sword += selected_count;
                            village.units.sword = selected_count;
                        }

                     troops_info.villages.sort(sort_by_heavy_desc);
                        for (let i = troops_info.villages.length; i > 0; i--) {
                            const village = troops_info.villages[i - 1];
                            const threshold = (user_input.heavy_count - troops_info.selected.heavy) / i;
                            const ratio = threshold < village.units.heavy ? threshold / village.units.heavy : 1.0;
                            const selected_count = Math.min(Math.round(ratio * village.units.heavy), village.units.heavy);
